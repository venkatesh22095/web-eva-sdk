...
function renderUserQuestion(question, userIconTemplate, conversation) {
  if (question) {
    return `<div class="message-bubble question" id = "${
...
function renderInAlphaThoughts(conversation) {
    ...
    if (node.nodeType === Node.TEXT_NODE) {
      ...
      while ((match = regex.exec(text)) !== null) {
        ...
        if (source) {
          ...
          const hasUrl =
            (ref.url && typeof ref.url === "string") || ref.isDirectUrl;

          // Get page number if available
          const pageNumber = ref.page_number || source.reference?.page_number;
          ...
          if (hasUrl) {
            ...
            tooltipContent += `<div class="source-footer">
              ...
              pageNumber || ""
            )}" data-chunk-title="${encodeHtml(source.title || "")}">
          ...
          if (!firstReferenceProcessed && idx === 0) {
            ...
            const duplicateRefNumber = document.createElement("span");
  ...
  rootNode.querySelectorAll(".bc-source-ref").forEach((ref) => {
    ...
    if (source) {
      ...
      if (iconContainer) {
        ...
        const isInTooltipFooter = iconContainer.closest(".source-footer");
...
function renderAssistantQuestion(conversation, assistantIconTemplate, props) {
  ...
  if (
    typeof transformedQuestion === "string" &&
    transformedQuestion.includes(startDelimiter) &&
    transformedQuestion.includes(endDelimiter)
  ) {
    const startIndex =
      transformedQuestion.indexOf(startDelimiter) + startDelimiter.length;
  ...
  console.log("transformedQuestion1111", transformedQuestion);
  ...
  if (transformedQuestion || thoughts?.length || (sources && sources.length)) {
    ...
    console.log("mainContent222", mainContent);
    ...
    console.log("mainContent333", mainContent);

    console.log("sourcesHtml", sourcesHtml);
    ...
    wrapper.innerHTML = `
            ...
            question && showFeedbackOption
              ? renderFeedbackSection(conversation, sources, props)
              : ""
          }
				</div>
			</div>
  ...
  return `<div class="bot-flex-wrapper answer-container">
				...
				<div class='answerCntr'>					
					<div class="assistant-question-container ${
            conversation?.status === "completed"
              ? "completed-assistant-question-container"
              : ""
          }">
						${transformedQuestion ? MessageRenderer(transformedQuestion) : ""}
					</div>
				</div>
			</div>`;
}
...
function renderConversationAgentIcon(data) {
  return `
		<div class='bot-conversation-icon-block'>                                    
                                        <span class='icon-block'><img src=${
                                          data?.sources?.[0]?.icon ||
                                          data?.agentIcon
                                        } alt="" /></span>
                                        <span class='bot-agent-name'>${
      ...
      if (conversation?.loading) {
        content += `<div class="message-bubble loading">
              ...
              assistantIconTemplate ? assistantIconTemplate : ""
						...
						<div class="message-container"><div class="loading-text">${encodeHtml(
              loadingText
            )}</div></div>
					</div>
...
function handleBotTemplates(conversation, props) {
  ...
  return `
		<div class="bot-scroll-container">
			<div class="bot-template-container botTemplate-${
        conversation?.messageId
      }" style="${
    conversation?.status === "completed" ? "pointer-events: none" : ""
  }"></div>
		</div>
	`;
}
...
function setupEventListeners(botConversation, props) {
  ...
  document.querySelectorAll(".expandAreaBlock").forEach((button) => {
    button.addEventListener("click", (event) => {
      ...
      const messageId =
        event.target.closest(".expandAreaBlock")?.dataset?.messageId;
      const isCollapsed =
        event.target.closest(".expandAreaBlock")?.dataset?.collapsed === "true";
      const contentDiv = document.querySelector(
        `.bot-conversation-content[data-message-id="${messageId}"]`
      );
      const summaryDiv = document.querySelector(
        `.bot-conversation-summary[data-message-id="${messageId}"]`
      );
      ...
      if (isCollapsed) {
        ...
        expandBlock.innerHTML = `${MinimizeIcon({
          size: 16,
          color: "#667085",
        })}`;
      } else {
        ...
        expandBlock.innerHTML = `${MaximizeIcon({
          size: 16,
          color: "#667085",
        })}`;
      }
  ...
  if (!isEmpty(botConversation)) {
    ...
    if (templateConversations?.length) {
      templateConversations.forEach((conversation) => {
        if (
          conversation?.templateType === "bot_template" &&
          conversation?.content?.payload?.template_type === "quick_replies"
        ) {
          if (
            !conversation.template_html?.querySelector(".quickRepliesTemplate")
          )
            conversation.template_html?.appendChild(
              renderQuickRepliesTemplate(conversation?.content?.payload)
            );
        }
...
const renderThoughts = (conversation) => {
  ...
  return `
    ...
    }' id='${uniqueId}' data-toggle-thoughts>
			<span class='thoughts-header' id='thoughts-header-${
        conversation?.messageId
      }'>Thoughts ${
    isCollapsed
      ? `for ${
          conversation?.thoughts?.[conversation?.thoughts?.length - 1]
            ?.thoughtTime
        } secs`
      : ""
  }</span>
			<span class='spanIcon'>${cheveronRightIcon({
        size: 8,
        color: "#70707B",
      })}</span>
  ...
  let html = `
        <div class='query-response-flow-items'>
            ${conversation?.thoughts
              ?.map((thought, index) => {
                return `
		<div class='thoughtsContent' key=${index} style="animation-delay: ${
                  index * 0.2
                } s">
		...
		${
      !conversation?.hasOwnProperty("question")
        ? index === conversation?.thoughts?.length - 1
          ? `<div class='thought-loader-wrapper'>
...
const setupThoughtsToggle = (messageId, thoughtTime, retryCount = 0) => {
  ...
  const thoughtWrapper = document.getElementById(
    `thought-wrapper-${messageId}`
  );

  if (!thoughtWrapper) {
    if (retryCount < maxRetries) {
      setTimeout(
        () => setupThoughtsToggle(messageId, thoughtTime, retryCount + 1),
        retryDelay
      );
      return;
    }
  ...
  const toggleThoughts = (event, thoughtTime) => {
    ...
    const thoughtsHeader = document.getElementById(
      `thoughts-header-${messageId}`
    );
...
function setupSourcesAccordionListeners() {
  ...
  if (!window.sourcesMutationObserver) {
    window.sourcesMutationObserver = new MutationObserver(function (mutations) {
  ...
  wrapper.addEventListener("click", function (e) {
    ...
    if (downloadBtn) {
      ...
      const isDownloadable = ["xls", "xlsx", "csv", "ppt", "pptx"].includes(
        fileExtension
      );
...
function renderBotConversation(
  ...
  const conversationWithThoughts = Object.values(botConversation || {}).find(
    (conv) => conv?.thoughts?.length > 0
  );
  ...
  return `
        ...
        }" data-message-id="${props?.messageId} id="bot-conversation-wrapper">
		${conversationWithThoughts ? renderThoughts(conversationWithThoughts) : ""}
			<div class="bot-conversation-content-wrapper ${
        props?.status === "completed" ? " bot-conversation-completed" : ""
      }">
				<div class="expand-bot-conversation ${
          props?.status === "completed" ? " conversation-completed" : ""
        }">
				<div class="top-header">
					...
					<div class="expandAreaBlock" data-message-id="${
            props?.messageId
				...
				<div class="bot-conversation-summary" data-message-id="${
          props?.messageId
        }" style="display: none;">
  ...
  ref.addEventListener("mouseenter", function (e) {
    ...
    if (tooltip && !isTooltipVisible) {
      ...
      setTimeout(() => {
        ...
        const tooltipHeight = Math.min(tooltipRect.height, maxTooltipHeight);
...
export function render(
  ...
  function ensureTooltipListenersAttached(attempt = 0) {
    ...
    if (tooltipRefs.length > 0) {
      ...
      requestAnimationFrame(() => ensureTooltipListenersAttached(attempt + 1));
    }
  }
      ...
      mutations.forEach((mutation) => {
        mutation.addedNodes.forEach((node) => {
          if (node.nodeType === 1) {
            ...
            if (
              node.classList?.contains("thought-wrapper") ||
              node.querySelector?.(".thought-wrapper")
            ) {
              setupAllThoughtToggles();
            }
          }
